...
def claim_has_action_with_due_date(claim: dict, action_name: str) -> bool:
    for action in claim_available_actions(claim):
        if action.get("action") == action_name and action.get("due_date"):
            return True
    return False
...
def build_ml_devolucao(claim: dict, sync_run_id: int | None = None) -> dict:
    ...
    # Ação de "send_message_to_mediator" isolada não significa que a revisão da plataforma terminou.
    ...
    if ml_review_locked:
        ...
        prazo = None
        requer_acao = 0
        acao = "Devolucao em revisao pelo Mercado Livre. Ainda nao e possivel abrir chamado/mediacao. Aguarde o termino da revisao da plataforma."
...
def ml_review_locked_item(item: dict | sqlite3.Row) -> bool:
    get = (lambda key, default="": item[key] if isinstance(item, sqlite3.Row) else item.get(key, default))
    requer_acao = int(get("requer_acao", 1) or 0)
    if requer_acao == 1:
        return False
    return_status = str(get("ml_return_status", "") or "").lower()
    seller_status = str(get("ml_seller_status", "") or "").lower()
    logistic = str(get("ml_tipo_logistica", "") or "").lower()
    review_keywords = ("review", "revis")
    explicit_review_statuses = {"in_review", "under_review", "pending_review", "reviewing"}
    has_review_signal = (
        return_status in explicit_review_statuses
        or seller_status in explicit_review_statuses
        or any(key in return_status for key in review_keywords)
        or any(key in seller_status for key in review_keywords)
    )
...
def apply_ml_queue_window(rows: list[dict]) -> None:
    ...
    # Sem corte artificial: manter todos os itens que batem na regra de classificação.
    return
    ...
    with db() as conn:
        ...
        if ml_review_locked_item(devolucao):
            return jsonify(
                {
                    "mensagem": "Devolucao ainda em revisao pelo Mercado Livre.",
                    "orientacao": "Ainda nao e possivel abrir contestacao/mediacao para este pedido.",
                }
            ), 409
        cur = conn.execute(
            """
